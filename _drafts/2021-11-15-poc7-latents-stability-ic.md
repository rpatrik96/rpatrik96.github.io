...
X &= f_X(U_X) \\
Z &= f_Z(X, U_Z) \\
Y &= f_Y(X, Z, U_Y) = f_Y(X, f_Z(X, U_Z) , U_Y) = f'_Y(X, U_Z, U_Y)\\
\end{aligned}
$$

- this dependency means that we would have an edge in O, thus, no need for the latent
...
>A latent structure $L_{[O]} = <G_{[O]}, O>$ is a **projection** of another latent structure $L$ if and only if:
> 1. Every unobservable variable $Z$ of $G_{[O]}$ is the middle node of a fork $X\leftarrow Z \rightarrow Y$, where $X$ and $Y$ are observed; and
> 2. For every stable distribution $P$ generated by $L$, there exists a stable distribution $P'$ generated by $L_{[O]}$ such that $I(P_{[O]}) = I(P'_{[O]})$
