...
**Steps**
  ...
  3. In the resulting PDAG (Partially Directed Acyclic Graph), orient **and mark** as many of the undirected edges as possible subject to two rules:
      1. **Rule $1^{*}$**: Add an arrowhead to $a\rightarrow c - b$ to get $a\rightarrow c\rightarrow^* b$ for nonadjacent $a$ and $c$.
...
**Rule $1^{*}$** is _almost_ the same as **Rule 1** in $IC$, but there are two differences: first, the $a\rightarrow c$ edge here comprises of $a\rightarrow^{*} c$ and $a \leftarrow L \rightarrow c$; second, $\rightarrow^{*}$ is used to indicate the directed edge from $c$ to $b$. Thus, it includes the case when a latent variable leaves the path between $a$ and $c$ active.
